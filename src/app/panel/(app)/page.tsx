  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-3">
        <div className="flex min-h-[140px] flex-col items-center justify-center rounded-2xl border border-stone-200 bg-stone-50 p-5 shadow-sm text-center transition hover:border-stone-300 hover:bg-white">
          <p className="text-xs font-bold uppercase tracking-wide text-stone-500">Saldo monedero</p>
          <p className="mt-2 text-2xl font-bold text-stone-900">{Number(balance).toFixed(2)} €</p>
        ...
        <div className="flex min-h-[140px] flex-col items-center justify-center rounded-2xl border border-stone-200 bg-stone-50 p-5 shadow-sm text-center transition hover:border-stone-300 hover:bg-white">
          ...
          <p className="mt-2 text-2xl font-bold text-stone-900">{bookings?.length ?? 0}</p>
          <Link href="/panel/reservas" className="mt-2 inline-block text-xs font-bold text-[#1d4ed8] hover:text-[#1e40af]">
            Ver todas →
          </Link>
        </div>
        <div className="col-span-2 flex min-h-[140px] flex-col items-center justify-center rounded-2xl border border-stone-200 bg-stone-50 p-5 shadow-sm text-center transition hover:border-stone-300 hover:bg-white lg:col-span-1">
          <p className="text-xs font-bold uppercase tracking-wide text-stone-500">Depósito por reserva</p>
          <p className="mt-2 text-2xl font-bold text-stone-900">4,50 €</p>
            ...
            <ul className="divide-y divide-stone-200 text-sm">
              {bookings.map((b: { id: string; booking_date: string; start_time: string; end_time: string; status: string; courts: { name: string } | { name: string }[] | null }) => {
                ...
                <li key={b.id} className="flex items-center justify-between gap-4 px-4 py-3.5 transition hover:bg-stone-50">
                  ...
                  <span className="inline-flex rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-bold text-emerald-700">Confirmada</span>
                </li>
                );
              })}
            </ul>
