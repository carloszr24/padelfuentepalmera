...
type AdminCourtBlocksSectionProps = {
  showBackToPistasLink?: boolean;
};

export async function AdminCourtBlocksSection({ showBackToPistasLink = true }: AdminCourtBlocksSectionProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-[10px] bg-[#f7f7f5] p-5">
        <h2 className="admin-stat-label mb-2">Bloqueos puntuales por fecha</h2>
        <p className="mb-4 text-[13px] text-[#6b6b6b]">
          Estas franjas desaparecen del panel de reservas de los usuarios en la fecha indicada.
        </p>
        {courtList.length === 0 ? (
