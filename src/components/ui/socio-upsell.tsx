...
export function SocioUpsell({ showPayButton = false, ctaLabel, onCtaClick }: SocioUpsellProps) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // #region agent log
      try{navigator.sendBeacon('http://127.0.0.1:7543/ingest/b946c3ce-2e52-4378-b9f6-afbd4bfaf00a',new Blob([JSON.stringify({sessionId:'68ad37',location:'socio-upsell:create-membership-response',message:'Membership API response',data:{status:res.status,ok:res.ok,error:data?.error,hasFormAction:!!data?.formAction,hasFormFields:!!data?.formFields},timestamp:Date.now(),hypothesisId:'B-C'})],{type:'application/json'}));}catch(_){}
      // #endregion
      if (!res.ok) {
      ...
      if (data.formAction && data.formFields && typeof data.formFields === 'object') {
        // #region agent log
        try{navigator.sendBeacon('http://127.0.0.1:7543/ingest/b946c3ce-2e52-4378-b9f6-afbd4bfaf00a',new Blob([JSON.stringify({sessionId:'68ad37',location:'socio-upsell:form-submit',message:'Submitting membership to Cecabank',data:{formAction:data.formAction,Importe:data.formFields?.Importe,hasF:!!data.formFields?.Firma,keys:Object.keys(data.formFields||{})},timestamp:Date.now(),hypothesisId:'E'})],{type:'application/json'}));}catch(_){}
        // #endregion
